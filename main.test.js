const { getDependencyGraphURL, URL_MAX_LENGTH } = require("./main");

describe("getDependencyGraphURL", () => {
  it("should yield an error if `itemRows` is not defined", () => {
    expect(() => getDependencyGraphURL()).toThrow(/itemRows/);
  });

  it("should yield an error if `dependsOnRows` is not defined", () => {
    expect(() => getDependencyGraphURL(["a", "b"])).toThrow(/dependsOnRows/);
  });

  it("should yield an error if `edgeRepresentation` is not a valid value", () => {
    expect(() =>
      getDependencyGraphURL(["a", "b", "c"], ["", "a,c"], "plop")
  ...
  it("should yield an error if the generated graph URL is larger than URL_MAX_LENGTH", () => {
    expect(() => getDependencyGraphURL(items, dependsOn)).toThrow(
      /URL_MAX_LENGTH/
    );
  });
